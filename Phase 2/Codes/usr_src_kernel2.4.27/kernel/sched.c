...
/* ====== LOTTERY SCHEDULER CONSTANTS AND VARIABLES ====== */
#define LOTTERY_ENABLED 1
#define DEFAULT_SCHEDULER 0

/*
Global variable for selected scheduler. 
Our system call will use this.
*/

int current_scheduler = DEFAULT_SCHEDULER;
/* ====== END LOTTERY SCHEDULER ====== */
...
/* ====== LOTTERY SCHEDULER HELPER FUNCTIONS ====== */
/*
There are more than 1 function in this part.*/



/*
 * Update tickets based on time rules (C and D parameters) but in here we will use LOSE_TICKET_TIME and GAIN_TICKET_TIME
 * Called before ticket counting or process selection in order to prevent starvation.
 * 
 */
static void update_tickets_based_on_time(struct task_struct *p)
{
    unsigned long current_time = jiffies;
    unsigned long time_diff_jiffies;
    unsigned long time_diff_ms;
    
    if (p->last_cpu_time == 0) {
        return;
    }
    
    time_diff_jiffies = current_time - p->last_cpu_time;
    
    time_diff_ms = (time_diff_jiffies * 1000) / HZ;
    
    if (time_diff_ms < LOSE_TICKET_TIME) {
        if (p->lottery_tickets > MIN_TICKETS) {
            p->lottery_tickets--;
        }
    } else if (time_diff_ms > GAIN_TICKET_TIME) {
        if (p->lottery_tickets < MAX_TICKETS) {
            p->lottery_tickets++;
        }
    }
}
...
/*
As in the name it gets total tickets of the all processes that exist in the system
*/

static int get_total_tickets(void)
{
    ...
    for_each_task(p) {
        if (p->state == TASK_RUNNING) {
            update_tickets_based_on_time(p);
 ...
 * Select next process using Lottery Scheduling:
	Lottery Scheduling works like this:
	1- We will generate a "winning" number of tickets. This "winning" tickets number is between 0 and total_tickets-1.
	2- If any of the processes have more than "winning" number of tickets, it will executed.
	3- Returns the selected process OR NULL if none of the process is found
 */
static struct task_struct *select_by_lottery(void)
{
    struct task_struct *p;
    ...
    winning_ticket = (int)(((unsigned long)jiffies * 1103515245 + 12345) % total_tickets);
    
    current_ticket = 0;
    for_each_task(p) {
        if (p->state == TASK_RUNNING) {
            current_ticket += p->lottery_tickets;
            if (current_ticket > winning_ticket) {
                p->last_cpu_time = jiffies;
                return p;
            }
        }
    }
...
/* ====== SCHEDULER SELECTION ====== */
/*	
	This part is one of the crucial part. 
	We are checking the current_scheduler flag that tells us the current scheduler.
	if Lottery Scheduler is enabled, processes wil be executed according to Lottery Scheduler
	if not enabled, processes will be executed according to Default Scheduler.
	
	With this configuration, we will be able to set the scheduler dynamically.
*/
	if (current_scheduler == LOTTERY_ENABLED) {
		next = select_by_lottery();
...
/* ====== LOTTERY SCHEDULER SYSTEM CALL ====== */
/*
	This is the System call and it has only 1 job:
	It will behave like a Lottery Scheduler switch.
	if user enter 1 => Turn on the Lottery Scheduler
	if user enter 0 => Turn of the Lottery Scheduler so it will turning on the Default Scheduler
*/
asmlinkage long sys_set_scheduler(int mode)
{
    if (mode == LOTTERY_ENABLED) {
        current_scheduler = LOTTERY_ENABLED;
        printk("Lottery scheduler enabled\n");
    } else if (mode == DEFAULT_SCHEDULER) {
        current_scheduler = DEFAULT_SCHEDULER;
        printk("Default scheduler enabled\n");
    } else {
        return -EINVAL;  /* Invalid argument */
    }
    return 0;
}
/* ====== END LOTTERY SYSTEM CALL ====== */
